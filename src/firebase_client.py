...
class FirebaseClient:
    """Firestore ile e-posta görevleri oluşturmak için istemci."""

    def __init__(self):
        """Firebase bağlantısını hazırlar."""
        try:
            if not os.path.exists(FIREBASE_CONFIG_PATH):
                raise FileNotFoundError(
                    f"Firebase yapılandırma dosyası bulunamadı: {FIREBASE_CONFIG_PATH}"
                )
    ...
    def add_email_task(self, recipient: str, subject: str, body: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"E-posta görevi başarıyla Firestore'a eklendi. Görev ID: {doc_ref.id}"
            )
            return doc_ref.id
        except Exception as e:
